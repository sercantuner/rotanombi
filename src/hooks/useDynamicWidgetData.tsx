...
function applyWidgetFilters(data: any[], widgetFilters: WidgetLocalFilters, diaAutoFilters?: DiaAutoFilter[]): any[] {
  ...
  // Dinamik/custom filtre key'leri - yukarıdaki hardcoded alanlar dışında kalan key'ler
  // Widget builder'da tanımlanan özel filtreler (kullaniciadi, personel vb.) burada uygulanır
  ...
  for (const [key, value] of Object.entries(wf)) {
    if (handledKeys.has(key)) continue; // Zaten yukarıda işlendi
    if (value === undefined || value === null) continue;
    if (typeof value === 'string' && !value.trim()) continue;
    if (value === 'hepsi') continue;
    
    // Multi-select (dizi) filtreler
    if (Array.isArray(value)) {
      if (value.length === 0) continue;
      // Veri setinde bu alan var mı kontrol et
      if (!dataHasField(filtered, [key])) continue;
      filtered = filtered.filter(row => {
        const rowVal = row[key];
        if (rowVal === null || rowVal === undefined) return false;
        // Nested obje desteği (unvan, aciklama gibi)
        const displayVal = typeof rowVal === 'object' && rowVal !== null
          ? (rowVal.unvan || rowVal.aciklama || rowVal.adi || rowVal.label || rowVal.name || String(rowVal))
          : String(rowVal);
        return value.includes(displayVal);
      });
      continue;
    }
    
    // Toggle (boolean) filtreler
    if (typeof value === 'boolean') {
      if (!dataHasField(filtered, [key])) continue;
      filtered = filtered.filter(row => {
        const rowVal = row[key];
        return Boolean(rowVal) === value;
      });
      continue;
    }
    
    // Number filtreler (min/max gibi)
    if (typeof value === 'number') {
      if (!dataHasField(filtered, [key])) continue;
      filtered = filtered.filter(row => {
        const rowVal = parseFloat(String(row[key])) || 0;
        return rowVal >= value;
      });
      continue;
    }
    
    // Dropdown (string) filtreler

    ...
    <div className="glass-card rounded-xl p-6 animate-slide-up">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {hasActiveFilters 
              ? <><span className="text-primary font-medium">{filteredCariler.length}</span> / {cariler.length} müşteri (filtrelenmiş)</>
              : `${cariler.length} müşteri · Bakiye bazlı sıralama`
            }
          </p>
        </div>
      ...
      {topCariler.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p>{hasActiveFilters ? 'Filtre kriterlerine uyan müşteri bulunamadı' : 'Cari verisi bulunamadı'}</p>
        </div>
      ) : (
        <div className="space-y-4">
          {topCariler.map((cari, index) => {
            ...
            return (
              <div key={cari._key || index} className="space-y-2 p-3 rounded-lg bg-secondary/20 hover:bg-secondary/40 transition-colors">
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {cari.vadesigecentutar > 0 && (
                        <span title={`Vadesi geçmiş: ${formatCurrency(cari.vadesigecentutar)}`}>

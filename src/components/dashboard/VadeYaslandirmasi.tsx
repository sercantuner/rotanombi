...
export function VadeYaslandirmasi({ yaslandirma, isLoading, widgetId }: Props) {
  ...
  const { chartData, toplam, gecmisToplam, gelecekToplam } = useMemo(() => {
    if (periyot === 'gunluk') {
      ...
      const data = [
        { name: '4+ Hafta', value: yaslandirma.vade90Plus + yaslandirma.vade90 * 0.5, key: 'hafta4plus', type: 'gecmis', color: getColor(0) },
        { name: '3 Hafta', value: yaslandirma.vade90 * 0.5 + yaslandirma.vade60 * 0.5, key: 'hafta3', type: 'gecmis', color: getColor(1) },
        { name: '2 Hafta', value: yaslandirma.vade60 * 0.5 + yaslandirma.vade30 * 0.5, key: 'hafta2', type: 'gecmis', color: getColor(2) },
        { name: '1 Hafta', value: yaslandirma.vade30 * 0.5, key: 'hafta1', type: 'gecmis', color: getColor(3) },
        { name: 'BU HAFTA', value: yaslandirma.guncel, key: 'buhafta', type: 'guncel', color: getColor(4) },
        { name: '+1 Hafta', value: yaslandirma.gelecek30 * 0.5, key: 'gelecek1', type: 'gelecek', color: getColor(5) },
        { name: '+2 Hafta', value: yaslandirma.gelecek30 * 0.5 + yaslandirma.gelecek60 * 0.5, key: 'gelecek2', type: 'gelecek', color: getColor(6) },
        { name: '+3 Hafta', value: yaslandirma.gelecek60 * 0.5 + yaslandirma.gelecek90 * 0.5, key: 'gelecek3', type: 'gelecek', color: getColor(7) },
        { name: '+4+ Hafta', value: yaslandirma.gelecek90 * 0.5 + yaslandirma.gelecek90Plus, key: 'gelecek4plus', type: 'gelecek', color: getColor(0) },
      ];
      const toplam = data.reduce((acc, item) => acc + item.value, 0);
      ...
      const data = [
        { name: '3+ Ay', value: yaslandirma.vade90Plus, key: 'ay3plus', type: 'gecmis', color: getColor(0) },
        { name: '2 Ay', value: yaslandirma.vade90 + yaslandirma.vade60, key: 'ay2', type: 'gecmis', color: getColor(1) },
        { name: '1 Ay', value: yaslandirma.vade30, key: 'ay1', type: 'gecmis', color: getColor(2) },
        { name: 'BU AY', value: yaslandirma.guncel, key: 'buay', type: 'guncel', color: getColor(3) },
        { name: '+1 Ay', value: yaslandirma.gelecek30, key: 'gelecekay1', type: 'gelecek', color: getColor(4) },
        { name: '+2 Ay', value: yaslandirma.gelecek60 + yaslandirma.gelecek90, key: 'gelecekay2', type: 'gelecek', color: getColor(5) },
        { name: '+3+ Ay', value: yaslandirma.gelecek90Plus, key: 'gelecekay3plus', type: 'gelecek', color: getColor(6) },
      ];
      const toplam = data.reduce((acc, item) => acc + item.value, 0);
      const gecmis = data.filter(d => d.type === 'gecmis').reduce((acc, d) => acc + d.value, 0);
      const gelecek = data.filter(d => d.type === 'gelecek').reduce((acc, d) => acc + d.value, 0);
  ...
  const handleBarClick = (data: any) => {
    if (data && data.key) {
      setSelectedBar(prev => prev === data.key ? null : data.key);
    }
  };
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-card border border-border rounded-lg shadow-lg p-3 z-50">
          ...
          <p className="text-md font-semibold text-accent">%{percentage}</p>
          ...
          <div className="flex items-center bg-secondary/50 rounded-lg p-0.5 md:p-1">
            <button
              ...
              className={`px-2 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-medium rounded-md transition-colors ${
                periyot === 'gunluk' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
              }`}
            >Gün</button>
            <button
              onClick={() => setPeriyot('haftalik')}
              className={`px-2 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-medium rounded-md transition-colors ${
                periyot === 'haftalik' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
              }`}
            >Hafta</button>
            <button
              onClick={() => setPeriyot('aylik')}
              className={`px-2 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-medium rounded-md transition-colors ${
                periyot === 'aylik' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
              }`}
            >Ay</button>
          </div>
      ...
      <div className="h-40 md:h-56">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 10, right: 5, left: 0, bottom: 5 }}>
            ...
            <Bar dataKey="value" radius={[3, 3, 0, 0]} cursor="pointer" onClick={(data) => handleBarClick(data)}>
              {chartData.map((entry, index) => (
                <Cell 
                  key={`cell-${index}`} 
                  fill={entry.color}
                  opacity={selectedBar && selectedBar !== entry.key ? 0.3 : 1}
                  stroke={selectedBar === entry.key ? 'hsl(var(--foreground))' : 'none'}
                  strokeWidth={selectedBar === entry.key ? 2 : 0}
                />
              ))}
            </Bar>
          </BarChart>
        ...
        <div className="mt-2 md:mt-4 flex items-center justify-between p-2 md:p-3 rounded-lg bg-primary/10 border border-primary/30">
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <span className="text-xs md:text-sm font-bold text-primary">
              {chartData.find(d => d.key === selectedBar)?.name || selectedBar}
            </span>
          </div>
          <button 

...
export function DashboardFilters({ totalCustomers }: Props) {
  const { 
    filters, 
    setFilter, 
    clearFilters, 
    ...
    <div className="glass-card rounded-xl p-4 mb-6 animate-fade-in">
      <div className="flex flex-wrap items-center gap-4">
        ...
        <Badge variant="outline" className="h-9 px-4 gap-2 text-sm font-semibold">
          <span className="text-primary">{totalCustomers}</span>
          <span className="text-muted-foreground font-normal">Müşteri</span>
        </Badge>
      ...
      {/* Active filter chips */}
      {(filters.ozelkod2.length > 0 || filters.satisTemsilcisi.length > 0 || filters.vadeDilimi) && (
